"""Synchronous keystore API.

This module contains the synchronous `KeyStore` implementation.

The async equivalent lives in `johnnycanencrypt.async_db.keystore.AsyncKeyStore`.
Both async and sync keystores should keep consistent naming and method patterns
...
from .backend import SqliteBackend, load_db_config
...
class Key:
    """In-memory representation of a public/secret OpenPGP key."""
...
class KeyStore:
    """Directory-backed keystore with a synchronous DB backend."""
                    ...
                    if len(uid["certifications"]) > 0:
                        for ucert in uid["certifications"]:
                            ...
                            for citem in ucert["certification_list"]:
                                # citem is like [('fingerprint', '...'), ('keyid', '...')]
    ...
    def __contains__(self, other: Union[str, Key]) -> bool:
        """Checks if a Key object or fingerprint str exists in the keystore or not.
    ...
    def update_expiry_in_subkeys(
        ...
        newcert = rjce.update_subkeys_expiry_in_cert(key.keyvalue, subkeys, etime, password)
        ...
        with con:
            ...
            cursor.execute("SELECT id from keys where fingerprint=?", (key.fingerprint,))
            fromdb = cursor.fetchone()
            _key_id = fromdb[0]
            # Now let us add the subkey and keyid details
    ...
    def revoke_userid(self, key: Key, userid: str, password: str) -> Key:
        ...
        newcert = rjce.revoke_uid_in_cert(key.keyvalue, userid.encode("utf-8"), password)
        ...
        with con:
            ...
            # Now loop through the new userids and find the new one
            cursor.execute(sql, (key.fingerprint, userid))  # Now we will mark this userid as revoked
            fromdb = cursor.fetchone()
            value_id = fromdb[0]
    ...
    def get_key(self, fingerprint: str) -> Key:
        """Finds an existing public key based on the fingerprint."""
    ...
    def get_keys(self, qvalue: str, qtype: str = "email") -> List[Key]:
        """Finds existing keys based on e-mail, name, uid value, or uri."""
    ...
    def sign_detached(self, key: Union[str, Key], data: Union[str, bytes], password):
        ...
        if k.keytype == KeyType.PUBLIC and k.oncard is not None:
            return rjce.sign_bytes_detached_on_card(k.keyvalue, data, password.encode("utf-8"))
    ...
    def sign_file_detached(
        ...
        Writes a signature to disk if `write=True`.
        """
        signature = ""
        if isinstance(key, str):  # Means we have a fingerprint
            k = self.get_key(key)
    ...
    def verify_file_detached(
        ...
        if not os.path.exists(signature_path):
            raise FileNotFoundError(f"The signature file at {signature_path} is missing.")
    ...
    def verify_and_extract_bytes(self, key: Union[str, Key], data: Union[str, bytes]) -> bytes:
        """Verifies the given data and returns the actual data."""
        if isinstance(key, str):  # Means we have a fingerprint
            k = self.get_key(key)
        else:
            k = key
    ...
    def _internal_fetch_from_server(self, url: str, term: str) -> Key:
        ...
        if resp.status_code == 404:
            raise KeyNotFoundError(f"The given search term {term} was found in the server.")
